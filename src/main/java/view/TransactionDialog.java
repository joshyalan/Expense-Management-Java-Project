...
public class TransactionDialog extends JDialog {
    ...
    private void initUI() {
        ...
        // Category
        gbc.gridy++;
        btnCategory = new JButton("Select Category");
        styleComponent(btnCategory);
        btnCategory.putClientProperty(FlatClientProperties.STYLE, "arc: 8; padding: 6,12,6,12; textAlign: left; background: " + hex(Theme.getBgPanel()));
        btnCategory.addActionListener(e -> openCategoryPicker());
        formCard.add(createLabeledField("Category", btnCategory), gbc);
        
        cbType.addActionListener(e -> {
            selectedCategory = null;
            btnCategory.setText("Select Category");
            btnCategory.setIcon(null);
        ...
        txtDesc.getDocument().addDocumentListener(new javax.swing.event.DocumentListener() {
            public void insertUpdate(javax.swing.event.DocumentEvent e) { autoSuggestCategory(); }
            public void removeUpdate(javax.swing.event.DocumentEvent e) { autoSuggestCategory(); }
            public void changedUpdate(javax.swing.event.DocumentEvent e) { autoSuggestCategory(); }
        });
    ...
    private void loadAvailableCategories() {
        ...
        availableCategories = categoryDAO.getCategoriesByUserAndType(userId, type);
    }
    
    private void autoSuggestCategory() {
        if (availableCategories == null || selectedCategory != null) return; // don't override manually selected
        String text = txtDesc.getText().toLowerCase().trim();
        if (text.isEmpty()) return;
        
        String suggestedName = null;
        if (text.contains("walmart") || text.contains("target") || text.contains("grocery") || text.contains("safeway")) suggestedName = "Groceries";
        else if (text.contains("netflix") || text.contains("spotify") || text.contains("hulu") || text.contains("disney")) suggestedName = "Subscriptions";
        else if (text.contains("uber") || text.contains("lyft") || text.contains("taxi") || text.contains("train")) suggestedName = "Transportation";
        else if (text.contains("shell") || text.contains("chevron") || text.contains("gas")) suggestedName = "Fuel";
        else if (text.contains("mcdonald") || text.contains("starbucks") || text.contains("restaurant") || text.contains("food")) suggestedName = "Food & Dining";
        else if (text.contains("amazon")) suggestedName = "Shopping";
    ...
    private void setCategoryUI(Category picked) {
        selectedCategory = picked;
        btnCategory.setText(picked.getName());
        btnCategory.setIcon(new Icon() {
            @Override
            public void paintIcon(Component c, Graphics g, int x, int y) {
                Graphics2D g2 = (Graphics2D) g.create();
                g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
                try { g2.setColor(Color.decode(picked.getColor())); } 
    ...
    private void openCategoryPicker() {
        ...
        List<Category> categories = categoryDAO.getCategoriesByUserAndType(userId, type);
        
        if (categories.size() < 10) {
            if (categoryDAO.seedDefaultCategories(userId)) {
                categories = categoryDAO.getCategoriesByUserAndType(userId, type);
                loadAvailableCategories(); // update cache
            }
        }
        
        CategoryPickerDialog dialog = new CategoryPickerDialog(this, categories, type);
        dialog.setVisible(true);
        
        Category picked = dialog.getSelectedCategory();
        if (picked != null) {
            if (picked.getId() == -1) {
                // It's a custom category
                Category newCat = new Category(userId, picked.getName(), type);
                newCat.setIcon("box");
                newCat.setColor("#8338EC");
                if (categoryDAO.addCategory(newCat)) {
                    loadAvailableCategories();
                    for (Category c : availableCategories) {
                        if (c.getName().equalsIgnoreCase(picked.getName())) {
                            setCategoryUI(c);
                            break;
                        }
                    }
    ...
    private void saveTransaction() {
        try {
            ...
            if (acc == null || selectedCategory == null) {
                ToastNotification.show((JFrame) getParent(), "Please select a valid Account and Category.", ToastNotification.WARNING);
                return;
            }
            ...
            if (amt <= 0) {
                ToastNotification.show((JFrame) getParent(), "Amount must be greater than 0.", ToastNotification.WARNING);
                return;
            }
            ...
            if (cbType.getSelectedItem().toString().equals("Expense")) {
                Expense ex = new Expense(userId, acc.getId(), selectedCategory.getId(), amt, dt, tm, desc, path);
                success = new ExpenseDAOImpl().addExpense(ex);
            } else {
                Income inc = new Income(userId, acc.getId(), selectedCategory.getId(), amt, dt, tm, desc, path);
