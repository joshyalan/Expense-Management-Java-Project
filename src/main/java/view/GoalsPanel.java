    ...
    private void showAddGoalDialog() {
        ...
        btnSave.addActionListener(e -> {
            try {
                ...
                if (name.isEmpty()) {
                    ToastNotification.show((JFrame) SwingUtilities.getWindowAncestor(this), "Goal name is required.", ToastNotification.WARNING);
                    return;
                }
                double target = Double.parseDouble(txtTarget.getText());
                if (target <= 0) {
                    ToastNotification.show((JFrame) SwingUtilities.getWindowAncestor(this), "Target amount must be greater than 0.", ToastNotification.WARNING);
                    return;
                }
