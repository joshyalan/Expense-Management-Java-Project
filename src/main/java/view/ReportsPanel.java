            ...
            if (chooser.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
                ...
                SwingWorker<Boolean, Void> worker = new SwingWorker<>() {
                    ...
                    protected Boolean doInBackground() throws Exception {
                        ...
                        List<Income> incomes = new IncomeDAOImpl().getAllIncomeByUser(userId);
                        
                        if (title.contains("PDF")) {
                            return ReportGenerator.generatePDFReport(finalPath, incomes, expenses);
                        } else {
                            return ReportGenerator.generateExcelReport(finalPath, incomes, expenses);
                        }
                    }
                    ...
                    protected void done() {
                        try {
                            boolean success = get();
                            if (success) {
                                ToastNotification.show((JFrame) SwingUtilities.getWindowAncestor(ReportsPanel.this), "Report saved successfully!", ToastNotification.SUCCESS);
                            } else {
                                ToastNotification.show((JFrame) SwingUtilities.getWindowAncestor(ReportsPanel.this), "Failed to save report.", ToastNotification.ERROR);
                            }
                        } catch (Exception ex) {
                            ToastNotification.show((JFrame) SwingUtilities.getWindowAncestor(ReportsPanel.this), "An error occurred.", ToastNotification.ERROR);
                        } finally {
                            btnExport.setEnabled(true);
                            btnExport.setText(btnText);
                        }
                    }
                };
                worker.execute();
